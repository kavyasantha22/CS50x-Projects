...
bool is_cycle(int winner, int loser, int origin);
...
void sort_pairs(void)
{
    for (int i = 0; i < pair_count - 1; i++)
    {
        ...
        for (int j = i + 1; j < pair_count; j++)
        {
            ...
            int max_pair_strength = preferences[pairs[max_index].winner][pairs[max_index].loser] -
                                    preferences[pairs[max_index].loser][pairs[max_index].winner];

            if (current_pair_strength > max_pair_strength)
            {
                max_index = j;
            }
        }
